<script>
let placeholder = "Search for anything";
...
function animatePlaceholder() {
    const currentPlaceholder = other_placeholders[currentIndex];

    if (isErasing) {
        // Erase text one character at a time
        if (textIndex > 0) {
            placeholder = currentPlaceholder.substring(0, textIndex - 1);
            textIndex--;
        } else {
            isErasing = false;
            currentIndex = (currentIndex + 1) % other_placeholders.length;
        }
    } else {
        // Write text one character at a time
...
// Animate with setInterval at a 100ms interval
...
<div class="container">
    ...
    <form class="search-form">
        <input type="text" class="search-input" placeholder={placeholder} on:keypress={
            (event) => {
                if (event.key === 'Enter') {
